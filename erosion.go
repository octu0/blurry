...
int liberosion(unsigned char *src, int32_t width, int32_t height, unsigned char *out) {
  ...
  int ret = erosion(in_rgba_buf, width, height, out_rgba_buf);
  free_buf(in_rgba_buf);
...
func Erosion(img *image.RGBA) (*image.RGBA, error) {
	width, height := wh(img)
