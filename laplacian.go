...
#cgo darwin LDFLAGS: -L${SRCDIR}/lib -lruntime_osx -llaplacian_osx -ldl -lm
#cgo linux  LDFLAGS: -L${SRCDIR}/lib -lruntime_linux -llaplacian_linux -ldl -lm
#include <stdlib.h>
#include <string.h>
...
int liblaplacian(unsigned char *src, int32_t width, int32_t height, unsigned char *out) {
  ...
  int ret = laplacian(in_rgba_buf, width, height, out_rgba_buf);
  free_buf(in_rgba_buf);
...
func Laplacian(img *image.RGBA) (*image.RGBA, error) {
	...
	if int(ret) != 0 {
		PutRGBA(out)
		return nil, errors.WithStack(ErrLaplacian)
	}
	return out, nil
}
